          ...
          <thead>
            <tr className="">
              {allColumns.map((column, index) => (
                <th
                  key={index}
                  className={`px-6  py-4 text-left ${index === 0 ? 'first:rounded-l-lg' : ''} ${index === allColumns.length - 1 ? 'last:rounded-r-lg ' : ''}`}
                  style={{
            ...
            {Object.keys(data[0]).map((key) => (
              ...
              >
                ...
                {allColumns.slice(1).map((column, index) => (
                  <td
                    key={index}
                    className={`px-6 py-4 justify-center ${index === allColumns.length - 1 ? 'last:rounded-r-lg ' : ''}`}
                  >
